...
if [[ "$MODE" != "formal" && "$MODE" != "quick" ]]; then
  echo "Uso: bash experiments/denoising/run_all.sh [formal|quick]"
  exit 1
fi

LOG_DIR="$ROOT_DIR/experiments/output/denoising/logs"
mkdir -p "$LOG_DIR"
...
run_experiment "noise_type" "experiments/denoising/noise_type.py"
run_experiment "noise_level" "experiments/denoising/noise_level.py"
